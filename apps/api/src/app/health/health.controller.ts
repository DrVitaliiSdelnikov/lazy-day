...
const MIGRATIONS: { name: string; sql: string }[] = [
  ...
  { name: '017', sql: `INSERT INTO event_sources (name, url, adapter_type, config) VALUES ('tkt.ge','https://tkt.ge/en/concerts','json_api','{"categories":[2,5],"note":"Public API key in frontend JS; categories: 2=concerts, 5=sports"}') ON CONFLICT (name) DO UPDATE SET url=EXCLUDED.url, adapter_type=EXCLUDED.adapter_type, config=EXCLUDED.config, enabled=true;` },
